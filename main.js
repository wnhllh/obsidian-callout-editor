			...
			if (rawContent[i].match(/<img.*?referrerpolicy="no-referrer">/g)) {
				...
				if (imgMatches) {
					imgMatches.forEach((imgTag) => {
						...
						// 替换原内容中的 <img> 标签
						rawContent[i] = rawContent[i].replace(
							imgTag,
							markdownImg
						);
					});
...
function HTMLtoList2(rawContent) {
	function processContentOptimized(htmlSegments) {
		...
		const fullHtml = htmlSegments
			.map((segment) =>
				segment.endsWith("<br>") ? segment : segment + "<br>"
			)
			.join("");
...
function HTMLtoList(rawContent) {
	...
	rawContent = rawContent.filter(
		(item) => !item.trim().startsWith("<button")
	);
	...
	for (let i = 0; i < rawContent.length; i++) {
		...
		if (rawContent[i].endsWith("<br>")) {
			rawContent[i] = rawContent[i].substring(
				0,
				rawContent[i].length - 4
			);
			if (rawContent[i].startsWith("<p>")) {
				rawContent[i] = rawContent[i].replace("<p>", "");
			}
...
var CalloutEditor = class {
	...
	async addCalloutContent(id, index) {
		...
		withoutScrollAndFocus(editorView, () => {
			...
			editorView.dispatch(
				setLineWithoutScroll(editor, lineNo + 2, ">> .")
			);
	...
	async updateChecklistItem(id, line, isChecked) {
		...
		withoutScrollAndFocus(editorView, () => {
			editorView.dispatch(
				setLineWithoutScroll(editor, line, newLineContent)
			);
		});
	}
		...
		withoutScrollAndFocus(editorView, () => {
			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (currentLine !== newLine) {
					changes.push(
						setLineWithoutScroll(editor, lineNum, newLine)
					);
				}
			}
		...
		withoutScrollAndFocus(editorView, () => {
			...
			for (let i = 0; i < newList.length; i++) {
				...
				if (newList[i].length > contentList[i].length) {
					...
					for (
						let a = 0;
						a < newList[i].length - contentList[i].length;
						a++
					) {
						changes.push(insertLineBelow(editor, baseLine + n - 1));
					}
					changes.push(
						setLineWithoutScroll(
							editor,
			...
			if (changes.length > 0) {
				editorView.dispatch({
					changes: changes.map((change) => ({
						from: change.changes.from,
						to: change.changes.to,
						insert: change.changes.insert,
					})),
					scrollIntoView: false,
				});
			}
		...
		class {
			...
			update(update) {
				...
				calloutEls.forEach((calloutEl) => {
					...
					const isInsideColumn = Array.from(columnsEls).some(
						(columnEl) => columnEl.contains(calloutEl)
					);
					...
					const calloutLine =
						update.state.doc.lineAt(calloutPos).number - 1;
					const calloutId = `callout_${calloutLine}`;
					calloutEl.id = calloutId;

					const calloutContent =
						calloutEl.querySelector(".callout-content");
			...
			setupEventListeners(view) {
				...
				// 	if (this.columnClickListener) {
				// 		contentDOM.removeEventListener('click', this.columnClickListener);
				// }

				const columnsEls = contentDOM.querySelectorAll(
					".callout" && "div[data-callout='multi-column']"
				);
				...
				columnsEls.forEach((columnEl) => {
					...
					const columnLine =
						view.state.doc.lineAt(columnPos).number - 1;
					...
					const columnContent =
						columnEl.querySelector(".callout-content");
					...
					const newHandler = ((columnId) => {
						return (e) => {
							...
							if (
								...
								let rawContent =
									columnContent.innerHTML.split("\n");
								rawContent = rawContent.filter(
									(line) =>
										!line.trim().startsWith("<div style=")
								);
			...
			setupColumnEl(columnEl, view) {
				...
				calloutContents.forEach((content, index) => {
					...
					// setupAddButton(content, columnId, index);
					...
					if (this.settings.enableSvgButton) {
						...
						const colorMenu = createColorMenu(
							colors,
							(selectedColor) => {
				...
				// function setupAddButton(content, columnId, index) {
				// 	if (!this.settings.enableAddButton) return;
...
var CalloutPlugin = class extends import_obsidian6.Plugin {
		...
		this.registerEvent(
			this.app.workspace.on("file-open", async () => {
				...
				this.app.workspace.onLayoutReady(() => {
					...
					// 在 CalloutPlugin 类的 onload 方法中修改注册 input 事件的部分
					...
					this.registerDomEvent(
						...
						this.debounce(async (e) => {
							...
							if (
								...
								if (
									calloutEl &&
									target.closest("." + onEditingClass)
								) {
									await this.saveChanges(calloutEl);
								}
								if (
									columnEl &&
									target.closest("." + onEditingMulti)
								) {
									await this.saveChanges2(columnEl);
								}
							}
					...
					this.registerDomEvent(
						...
						(e) => {
							const target = e.target;
							if (target.isContentEditable) {
								target.setAttribute("composing", "true");
							}
						}
					);
					...
					this.registerDomEvent(
						...
						async (e) => {
							const target = e.target;
							if (target.isContentEditable) {
								target.removeAttribute("composing");
								// 输入法输入完成后再触发一次更新
								const calloutEl = activeDocument.querySelector(
									"." + onEditingClass
								);
	...
	async saveChanges(calloutEl) {
		...
		if (calloutEl._lastContent === newContent) {
			// 内容没有变化，直接返回
			return;
		}

		// 更新缓存的内容
		calloutEl._lastContent = newContent;
		...
		await this.calloutEditor.updateCallout(
			calloutEl.id,
			content,
			rawContent
		);
	...
	async saveChanges2(calloutEl) {
		...
		await this.calloutEditor.updateCallout2(
			calloutEl.id,
			content,
			rawContent
		);
